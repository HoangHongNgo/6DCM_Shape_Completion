...
def loadObjModels(gn_root, objIds=None):
    ...
    **Output:**
    ...
    models = []
    for i in objIds:
        plyfile = os.path.join(gn_root, 'models', '%03d' %
                               i, 'nontextured.ply')
        models.append(o3d.io.read_point_cloud(plyfile))
    return models


def create_point_cloud_from_depth_image(depth, k, organized=True, s=1000.0):
    fx, fy = k[0, 0], k[1, 1]
    cx, cy = k[0, 2], k[1, 2]
    ...
    # scene point cloud in shape (720, 1280)
...
def ssd2pointcloud(cloud, mask, diff, format='img_rear'):
    ...
    uv[np.isnan(uv)] = 0
    # front view offset: front view unit vector * diff (||FR||)
