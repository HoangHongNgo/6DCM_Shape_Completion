""" 
    Tools for point cloud data processing.
    Author: chenxi-wang
"""
...
""" 
    The same as point cloud data processing
    but for tensors processing
"""


def create_point_cloud_from_depth_image_tensor(depth, fx, fy, cx, cy, scale, organized=True):
    """ Generate point cloud using depth image batch with shared camera intrinsics.
        Input:
            depth: [Tensor, (B, 1, H, W), torch.float32]
                depth image batch
            fx, fy, cx, cy, scale: [float or Tensor, (1, ), torch.float32]
                shared camera intrinsics for the entire batch
            organized: bool
                whether to keep the cloud in image shape (H,W,3)
        Output:
            cloud: [Tensor, (B, H, W, 3) / (B, HW, 3), torch.float32]
                generated cloud, (B, H, W, 3) for organized=True, (B, HW, 3) for organized=False
    """
    # Check for batch dimensions
    B, _, H, W = depth.shape

    # Create a grid of pixel coordinates (xmap, ymap)
    xmap, ymap = torch.meshgrid(torch.arange(
        H, device=depth.device), torch.arange(W, device=depth.device))

    # Expand to include batch size (B, H, W)
    xmap = xmap.unsqueeze(0).expand(B, -1, -1)
    ymap = ymap.unsqueeze(0).expand(B, -1, -1)

    # Get the depth values and normalize
    points_z = depth.squeeze(1) / scale  # Shape: (B, H, W)

    # Compute 3D coordinates (x, y, z) based on camera intrinsics
    points_x = (xmap - cx) * points_z / fx
    points_y = (ymap - cy) * points_z / fy

    # Stack to create the point cloud (B, H, W, 3)
    cloud = torch.stack([points_x, points_y, points_z], dim=-1)

    # Reshape the point cloud if not organized
    if not organized:
        cloud = cloud.view(B, -1, 3)  # Shape: (B, HW, 3)
